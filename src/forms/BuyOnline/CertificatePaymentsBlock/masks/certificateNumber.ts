...
const MaxSymbolsMask: number = 9;

export function certificateMask(): InputTextMask {
    return function(currentValue: string, prevValue: string, startCaretPosition: number, endCaretPosition: number ): InputMaskResult {
        ...
        if (raw.value.length > MaxSymbolsMask) {
            raw.value = raw.value.slice(0, MaxSymbolsMask);
